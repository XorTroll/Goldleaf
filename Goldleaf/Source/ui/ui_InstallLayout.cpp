...
namespace ui
{
    ...
    void InstallLayout::StartInstall(std::string Path, fs::Explorer *Exp, Storage Location, pu::Layout *Prev)
    {
        ...
        /*
        Result rc = nsp::Install(Path, Exp, Location, [&](ncm::ContentMetaType Type, u64 ApplicationId, std::string IconPath, NacpStruct *NACP, hos::TicketData *Tik, std::vector<ncm::ContentRecord> NCAs) -> bool
        {
            std::string info = set::GetDictionaryEntry(82) + "\n\n";
            switch(Type)
            {
            ...
            info += "\n";
            hos::ApplicationIdMask idmask = hos::IsValidApplicationId(ApplicationId);
            switch(idmask)
            {
                case hos::ApplicationIdMask::Official:
                    info += set::GetDictionaryEntry(87);
            ...
            info += "\n" + set::GetDictionaryEntry(90) + " " + hos::FormatApplicationId(ApplicationId);
            info += "\n\n";
            if(NACP != NULL)
            {
                NacpLanguageEntry *lent;
        ...
        {
            this->installBar->SetMaxValue(Total);
            std::string name = set::GetDictionaryEntry(148) + " \'"  + hos::GetStringFromNCAId(Record.NCAId);
            if(Record.Type == ncm::ContentType::Meta) name += ".cnmt";
            // name += ".nca\'... (NCA " + std::to_string(Content + 1) + " " + set::GetDictionaryEntry(149) + " " + std::to_string(ContentCount) + ")";
            name += ".nca\'... (" + fs::FormatSize(BytesSec) + "/s)";
            this->installText->SetText(name);
